...
function GetRandomGamemodeMap(gm)
    ...
    if gmtab then return table.Random(gmtab.maps) end
...
function GetNumberOfGamemodeMaps(gm)
    local gmtab = g_PlayableGamemodes[gm or GAMEMODE.FolderName]
    if gmtab then return table.Count(gmtab.maps) end
...
concommand.Add("votegamemode", function(pl, cmd, args)
    GAMEMODE:VotePlayGamemode(pl, args[1])
end)
...
concommand.Add("votemap", function(pl, cmd, args)
    GAMEMODE:VotePlayMap(pl, args[1])
end)
...
concommand.Add("VoteForChange", function(pl, cmd, args)
    GAMEMODE:VoteForChange(pl)
end)

timer.Create("VoteForChangeThink", 10, 0, function()
    if GAMEMODE then
        GAMEMODE.CountVotesForChange(GAMEMODE)
    end
end)
    ...
    if not GAMEMODE.m_bVotingStarted then
        ...
        if fretta_voting:GetBool() then
            if #g_PlayableGamemodes >= 2 then
                ...
                timer.Simple(fretta_votetime:GetFloat(), function()
...
function GM:StartMapVote()
    ...
    if GetNumberOfGamemodeMaps(GAMEMODE.WinningGamemode) == 1 then return GAMEMODE:FinishMapVote(true) end
...
function GM:WorkOutWinningGamemode()
    ...
    if not fretta_voting:GetBool() then return GAMEMODE.FolderName end
    local winner = GAMEMODE:GetWinningWant()
    if not winner then return GetRandomGamemodeName() end
...
function GM:GetWinningMap(WinningGamemode)
    ...
    local winner = GAMEMODE:GetWinningWant()
    if not winner then return GetRandomGamemodeMap(GAMEMODE.WinningGamemode) end
