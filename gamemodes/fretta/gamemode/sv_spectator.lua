...
function GM:IsValidSpectator(pl)
    ...
    if pl:Team() ~= TEAM_SPECTATOR and not pl:IsObserver() then return false end
...
function GM:IsValidSpectatorTarget(pl, ent)
    ...
    if not table.HasValue(GAMEMODE:GetValidSpectatorEntityNames(pl), ent:GetClass()) then return false end
    if ent:IsPlayer() and not ent:Alive() then return false end
    if ent:IsPlayer() and ent:IsObserver() then return false end
    if pl:Team() ~= TEAM_SPECTATOR and ent:IsPlayer() and GAMEMODE.CanOnlySpectateOwnTeam and pl:Team() ~= ent:Team() then return false end
...
function GM:BecomeObserver(pl)
    ...
    if not table.HasValue(GAMEMODE:GetValidSpectatorModes(pl), mode) then
...
local function spec_mode(pl, cmd, args)
    if not GAMEMODE:IsValidSpectator(pl) then return end
...
local function spec_next(pl, cmd, args)
    if not GAMEMODE:IsValidSpectator(pl) then return end
    if not table.HasValue(GAMEMODE:GetValidSpectatorModes(pl), pl:GetObserverMode()) then return end
