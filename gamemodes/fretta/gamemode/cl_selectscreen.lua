...
function PANEL:AddSelectButton(strName, fnFunction, txt)
    ...
    if txt then
        ...
        btn.OnCursorExited = function()
            self:SetHoverText(self.OldHoverText)
            self.OldHoverText = nil
        end
    end
...
function GM:ShowTeam()
    if not IsValid(TeamPanel) then
        ...
        for ID, TeamInfo in SortedPairs(AllTeams) do
            if ID ~= TEAM_CONNECTING and ID ~= TEAM_UNASSIGNED and (ID ~= TEAM_SPECTATOR or GAMEMODE.AllowSpectating) and team.Joinable(ID) then
        ...
        if IsValid(LocalPlayer()) and (LocalPlayer():Team() == TEAM_UNASSIGNED or LocalPlayer():Team() == TEAM_SPECTATOR or LocalPlayer():Team() == TEAM_HUNTERS or LocalPlayer():Team() == TEAM_PROPS) then
