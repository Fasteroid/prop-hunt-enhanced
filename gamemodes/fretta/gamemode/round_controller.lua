...
function GM:HasReachedRoundLimit(iNum)
    ...
    if iRoundLimit > 0 and iNum > iRoundLimit then return true end --MapVote.Start()
...
function GM:AddRoundTime(iAddedTime)
    if not GAMEMODE:InRound() then return end -- don't add time if round is not in progress
...
function GM:PreRoundStart(iNum)
    ...
    if not GAMEMODE:CanStartRound(iNum) then
        ...
        timer.Simple(1, function()
            GAMEMODE:PreRoundStart(iNum)
        end)

        return
    end

    timer.Create("RoundStartTimer", GAMEMODE.RoundPreStartTime, 1, function()
        GAMEMODE:RoundStart()
    end)
...
function GM:RoundEnd()
    ...
    timer.Simple(GAMEMODE.RoundPostLength, function()
        GAMEMODE:PreRoundStart(GetGlobalInt("RoundNumber") + 1)
    end)
...
function GM:CheckPlayerDeathRoundEnd()
    if not GAMEMODE.RoundBased then return end
...
function GM:CheckRoundEndInternal()
    ...
    timer.Create("CheckRoundEnd", 1, 0, function()
        GAMEMODE:CheckRoundEndInternal()
    end)
end
