...
function GM:TeamHasEnoughPlayers(teamid)
    ...
    if GetConVar("ph_forcejoinbalancedteams"):GetBool() then
        for id, tm in pairs(team.GetAllTeams()) do
            if id > 0 and id < 1000 and team.NumPlayers(id) < PlayerCount and team.Joinable(id) then return true end
        end
    end
...
function GM:PlayerFootstep(ply, pos, foot, sound, volume, rf)
    if GAMEMODE.NoPlayerFootsteps or not ply:Alive() or ply:Team() == TEAM_SPECTATOR or ply:IsObserver() then return true end
    local Class = ply:GetPlayerClass()
    if not Class then return end
    if Class.DisableFootsteps then return true end -- rather than using a hook, we can just do this to override the function instead.
    if Class.Footstep then return Class:Footstep(ply, pos, foot, sound, volume, rf) end -- Call footstep function in class, you can use this to make custom footstep sounds
end
...
function GM:GetTimeLimit()
    if GAMEMODE.GameLength > 0 then return GAMEMODE.GameLength * 60 end
...
function GM:PlayerNoClip(pl, on)
    ...
    if GAMEMODE.PlayerCanNoClip or game.SinglePlayer() or GetConVar("sv_cheats"):GetBool() then return true end
    -- Don't if it's not.
