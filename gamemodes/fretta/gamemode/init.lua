...
function GM:AutoTeam(pl)
    if not GAMEMODE.AllowAutoTeam then return end
...
function GM:FindLeastCommittedPlayerOnTeam(teamid)
    ...
    if worstteamswapper then return worstteamswapper end
...
function GM:PlayerShouldTakeDamage(ply, attacker)
    if GAMEMODE.NoPlayerSelfDamage and IsValid(attacker) and ply == attacker then return false end
    if GAMEMODE.NoPlayerDamage then return false end

    if GAMEMODE.NoPlayerTeamDamage and IsValid(attacker) then
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
    ...
    if GAMEMODE.EnableFreezeCam and IsValid(attacker) and attacker ~= ply then
