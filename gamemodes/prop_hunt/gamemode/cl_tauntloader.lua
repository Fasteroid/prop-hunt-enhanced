...
-- this was breaking for some reason when we did it on tick 1
hook.Add("InitPostEntity", "PHRequestTaunts", function()
    requestTaunts()
end)
...
function PHE:GetTeamTaunt(teamid)
    if teamid == TEAM_PROPS then return PHE.TAUNTS.PROPS end
    if teamid == TEAM_HUNTERS then return PHE.TAUNTS.HUNTERS end
