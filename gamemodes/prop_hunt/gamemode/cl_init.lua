...
function PHEDrawPropselectHalos()
    ...
    if GetConVar("ph_cl_halos"):GetBool() and LocalPlayer():Team() == TEAM_PROPS and LocalPlayer():Alive() then
        local trace = {}
...
-- fix stupid nil index error
hook.Add("Think", "nilindexfix", function()
    if TEAM_PROPS ~= nil then
        tutormats = {
            [TEAM_PROPS] = Material("vgui/hud_control_help_props.png"),
            [TEAM_HUNTERS] = Material("vgui/hud_control_help_hunters.png")
        }
...
net.Receive("PH_ShowTutor", function()
    if GetConVar("ph_show_tutor_control"):GetBool() and LocalPlayer():Alive() then
...
PHE.FreezeCamSnd = {"misc/freeze_cam.wav", "misc/freeze_cam_sad1.wav"}
