...
hook.Add("PlayerInitialSpawn", "PHE.PrecacheSoundCore", function()
    ...
    local function PrecacheThis(path)
        if file.Exists(path, "GAME") then
            ...
            for _, snd in pairs(wav) do
                util.PrecacheSound(snd)
            end

            for _, sndm in pairs(mp3) do
                util.PrecacheSound(sndm)
            end
        end
    end
    ...
    if GetConVar("ph_precache_taunts"):GetBool() then
        timer.Simple(3, function()
            for _, ptaunts in pairs(PHE.PROP_TAUNTS) do
                util.PrecacheSound(ptaunts)
            end

            for _, htaunts in pairs(PHE.HUNTER_TAUNTS) do
                util.PrecacheSound(htaunts)
            end
        end)
    end
end)