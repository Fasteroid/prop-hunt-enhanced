...
ENT.sounds = {"prop_idbs/huntep4_bc44_pickup.wav", "prop_idbs/zavogant_pickup.wav"}
...
ENT.funclists = {
    function(pl)
        if not pl.ph_fastspeed then
            if not pl._OriginalWSpeed then
                pl._OriginalWSpeed = pl:GetWalkSpeed()
            end
...
local function ResetEverything()
    for _, v in pairs(player.GetAll()) do
        if IsValid(v) and v:Alive() then
            ...
            if v:Team() == TEAM_PROPS and v._OriginalWSpeed then
                v:SetWalkSpeed(v._OriginalWSpeed)
            end
